...
def get_saliency_map(pil_img, model_checkpoint, num_classes):
    ...
    data = data_transform(pil_img.resize((224,224))).unsqueeze(0).requires_grad_(True)
    print(data.shape)
    data = data.to(device)
    output = model(data)
    score = output.max()
    ...
    score.backward()
    grads = data.grad
    saliency_map, _ = torch.max(torch.abs(grads.cpu()), 1)
    saliency_map = 255*(saliency_map-saliency_map.min())/(saliency_map.max()-saliency_map.min())
    return  Image.fromarray(saliency_map.numpy().astype(np.uint8)[0], mode='L').resize(pil_img.size)
...
def tile(path, level, col, row, format):
    ...
    try:
        ...
        tile = get_saliency_map(tile, app.config['model_checkpoint'], 2)
    except ValueError:
...
if __name__ == '__main__':
    ...
    parser.add_option('-s', '--size', metavar='PIXELS',
                ...
                help='tile size [254]')
    parser.add_option('-X', '--checkpoint', metavar='model_checkpoint',
    ...
    try:
        ...
        pass
    app.config["model_checkpoint"] = opts.model_checkpoint
